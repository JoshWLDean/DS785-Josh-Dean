...
# Invert y (on_time_flag) so late orders become the positive class
y = 1 - y
...
on_time_count = y_train.value_counts().get(0, 0) 
late_count = y_train.value_counts().get(1, 0) 
...
cm = confusion_matrix(y_test, final_predictions) # Invert to make late orders show as the positive classe
...
predicted_prob_df = pd.DataFrame({
    'Predicted Probability': final_probs,
    'Actual': y_test,
